...
def main():
  ...
  if(len(sys.argv) != 6):
    print "Usage: lut_generate <# bits> <# fractions> <# lut entries> <half range> <sin|cos|asin|acos|sigmoid>"
    exit(1)
  ...
  # Positive part
  init = 0.0
  half_range = int(sys.argv[4])
  lut_entries = float(sys.argv[3])
  bits = int(sys.argv[1])
  frac = int(sys.argv[2])
  inc  = half_range / (lut_entries/2.0)
  func = sys.argv[5]
  ...
  #out_fn.write("assign out <= mem[index];\n")

  value = init + inc / 2.0
  index = 0
  while value <= half_range:
    curr_val  = calculate_function(func, value)
    fixed_val = convert2fixed(curr_val, bits, frac)
    #test_val  = convert2dec(fixed_val, bits, frac)

    out_fn.write("\t\t\t%s'd%s: out = %s'b%s;\n" % (str(int.bit_length(int(lut_entries)-1)), str(index), str(bits), fixed_val))  
  ...
  out_fn.write(converter_module(bits, frac, lut_entries, half_range))
pass;
